...
class ChoiceModel(ABC):
    ...
    def _post_fit(
        ...
        if not self.convergence and verbose > 0:
            print(
                "**** The optimization did not converge after {} "
    ...
    def _setup_design_matrix(self, X):
        ...
        if isvars:
            ...
            dummy = np.delete(
                dummy, np.where(self.alternatives == self.base_alt)[0], axis=1
            )
            Xis = X[:, ispos]
    ...
    def _format_choice_var(self, y, alts):
        ...
        if isinstance(y[0], (np.number, np.bool_)) and np.array_equal(
            y.reshape(N, J).sum(axis=1), np.ones(N)
        ):
            return y
        else:
            ...
            if np.array_equal(y1h.reshape(N, J).sum(axis=1), np.ones(N)):
                ...
                raise ValueError(
                    "inconsistent 'y' values. Make sure the "
...
def diff_nonchosen_chosen(X, y, scale, addit, avail):
    ...
    scale_d = (
        scale[~y].reshape(N, J - 1) - scale[y].reshape(N, 1)
        if scale is not None
        else None
    )
    addit = (
    ...
    addit_d = (
        addit[~y].reshape(N, J - 1) - addit[y].reshape(N, 1)
        if addit is not None
        else None
    )
